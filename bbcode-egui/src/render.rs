...
impl<'a> BBView<'a> {
    ...
    pub fn show(&self, ui: &mut egui::Ui, state: &mut BBState) {
        let doc = bbcode::parse(self.src);
        let blocks = build_blocks(&doc.children, &Style::default());
        let id_base = ui.id().with("bbcode-egui").with(self.src.len());
        ui.push_id(id_base, |ui| {
            ...
            render_blocks(ui, &blocks, &mut ctx);
        });
        show_image_window(ui.ctx(), state);
    }
}

fn render_blocks(ui: &mut egui::Ui, blocks: &[Block<'_>], ctx: &mut Ctx<'_>) {
    for block in blocks {
        render_block(ui, block, ctx);
    }
}

fn render_block(ui: &mut egui::Ui, block: &Block<'_>, ctx: &mut Ctx<'_>) {
    match block {
        ...
        Block::Code(text) => {
            Frame::group(ui.style())
                .inner_margin(6.0)
                .show(ui, |ui| {
                    ui.label(RichText::new(text.as_ref()).monospace());
                });
        }
...
fn render_paragraph(ui: &mut egui::Ui, inlines: &[Inline<'_>], ctx: &mut Ctx<'_>) {
